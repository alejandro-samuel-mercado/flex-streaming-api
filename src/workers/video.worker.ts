...
export const videoWorker = new Worker(
  ...
  async (job: Job) => {
    ...
    const onProgress = async (percent: number) => {
      ...
      // Persist to DB every 5% so admin panel shows progress even when BullMQ job is temporarily in delayed state
      if (percent - lastSavedProgress >= 5 || percent >= 100) {
        ...
        prisma.videoFile.update({ where: { id: videoFileId }, data: { progress: Math.round(percent) } })
          .catch(() => { /* non-fatal — don't crash worker if DB update fails */ });
      }
    };
