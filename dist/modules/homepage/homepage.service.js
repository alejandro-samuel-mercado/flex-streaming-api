...
class HomepageService {
    ...
    static async getHomepageData() {
        ...
        const siteConfig = await prisma_1.prisma.siteConfig.findMany();
        ...
        const [trending, recent, estrenos, freeContent, platforms, genres, contentTypes, plans, faqItems,] = await Promise.all([
            ...
            prisma_1.prisma.platform.findMany({
            ...
            // All genres
            prisma_1.prisma.genre.findMany({ orderBy: { name: 'asc' } }),
            // Content type counts
            prisma_1.prisma.content.groupBy({
                by: ['type'],
                where: activeContentWhere,
                _count: true,
            }),
            ...
            prisma_1.prisma.siteConfig.findFirst({ where: { key: 'faq_items' } }),
        ]);
        ...
        if (bannerStrategy === 'MANUAL' || (bannerStrategy === 'COMBINED' && explicitIds.length > 0)) {
            if (explicitIds.length > 0) {
                const manualItems = await prisma_1.prisma.content.findMany({
                ...
                featured = await prisma_1.prisma.content.findMany({
        ...
        if (bannerStrategy === 'AUTO_LATEST') {
            ...
            if (featured.length < bannerLimit) {
                const legacy = await prisma_1.prisma.content.findMany({
